...
var miarray = [{'id':'1', 'letra':'A', 'palabra':'silla', 'description': 'Mueble que se usa para sentarse...', 'check':null},{'id':'2', 'letra':'B', 'palabra':'alfabeto', 'description':'Grupo de letras...', 'check':null},
 ...
 $(document).ready(iniciar12);

function iniciar12(){

    $(".letraM").click(ambar12);
    $(".letraM").click(reloj12);
    $(".letraM").click(M); // inicio evento al clickar en el div A, al clikar se inicio la funcion A
...
}else if(segundos12>9){
document.getElementById('displayReloj').innerHTML = '00:' + segundos12;
}else{
document.getElementById('displayReloj').innerHTML = '00:' + '0' + segundos12;
     }
...
$(".letraM").stop(ambar12);
$(".letraM").css("background-color", "red");
$("#question").empty();
...
tiempo12 =  setTimeout(function(){reloj12()},1000);
}
segundos12--;
}
 ...
 $(".letraM").fadeIn(400).delay(280).fadeOut(400, ambar12);
  ...
  $("#respuesta").empty();
  var description12 = "Contiene la letra M: <br>"+miarray[12].description; 

   $("#question").append(description12);              //en el div question se añade la descripcion de la letra
   
    var boton12 = "<button type='button' id='boton'onclick='recibir12();'>Responder</button>";
    $("#form").append(boton12);
...
function recibir12(){                                    // se crea una función para agrupar las funcionalidades
        

var palabraM= document.getElementById("palabra").value;     // se recoge el valor escrito en el input
var responde = document.getElementById("respuesta").innerHTML = palabraM;  //en el div respuesta mostramos cuando nos interese el valor del input
 
 if (responde===miarray[12].palabra) {  // condicion: si el valor es igual a la palabra del array se eejecuta.....
  ...
  var color = $(".letraM").css("background-color", "#3ADF00"); // el div cambiara su estado actual de color por un verde
...
var responde = document.getElementById("respuesta").innerHTML = "INCORRECTA<br> La palabra correcta es :"+ miarray[12].palabra; // se mostrara un mensaje
      
$(".letraM").css("background-color", "red");  //el estado actual del div azul con la letra pasara a rojo
